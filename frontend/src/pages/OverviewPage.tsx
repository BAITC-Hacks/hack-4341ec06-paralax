...
export function OverviewPage({
  ...
  const source =
    result.data_source === "partner_excel" ? "IEK · Excel" : "Синтетический";

  return (
    <>
      <div className="page-heading">
        <div>
          <p className="eyebrow">ОБЗОР / {source.toUpperCase()}</p>
          <h1>Планирование закупок</h1>
          <p>Рекомендации рассчитаны backend по выбранному набору.</p>
        </div>
        <button
      ...
      <section className="summary-strip" aria-label="Текущий набор">
        ...
        <div>
          ...
          <strong>{result.as_of_date}</strong>
        </div>
        <div>
          <span>СТАТУС</span>
          <strong className="status-draft">
            {result.status === "draft" ? "Черновик" : "Утверждён"}
          </strong>
        </div>
        <p>
          {result.data_source === "partner_excel"
            ? "Остаток IEK взят из месячного среза; проверьте актуальность перед утверждением."
            : "Синтетические входные данные, расчёт выполнен алгоритмом."}
        </p>
      </section>
      ...
      <section className="metric-grid" aria-label="Сводка рекомендаций">
        <Metric
          ...
          value={quantity(rows.length)}
          note="все товары"
        />
        <Metric
          icon={<PackageCheck size={19} />}
          label="К пополнению"
          value={quantity(toOrder.length)}
          note="количество больше нуля"
        />
        <Metric
          icon={<CircleAlert size={19} />}
          label="Высокий приоритет"
          value={quantity(highRisk.length)}
          note="включая критичные"
        />
        <Metric
          icon={<Truck size={19} />}
          label="Поставщиков"
          value={quantity(supplierCount)}
          note="в черновике"
        />
      </section>
      ...
      <section className="surface overview-list">
        <div className="section-heading">
          <div>
            <p className="eyebrow">ТРЕБУЮТ ВНИМАНИЯ</p>
          ...
          <button
            ...
            onClick={onOpenRecommendations}
          >
            Все позиции <ArrowRight size={15} />
          </button>
        </div>
        {highRisk.length === 0 && (
          <p className="empty-search">Нет позиций с высоким приоритетом.</p>
        )}
        {highRisk.slice(0, 8).map((row) => (
          <button
            className="attention-row"
            type="button"
            key={row.sku}
            onClick={() => onOpenRecommendation(row.sku)}
          >
            ...
            <strong className="attention-quantity">
              {quantity(row.recommended_quantity)} {productUnit(row)}
            </strong>
