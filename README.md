...
- Отдельный серверный модуль OpenAI для объяснения рассчитанных факторов с проверкой ответа и резервным текстом. **В интерфейсе маршрут объяснения пока возвращает резервный текст:** адаптер к этому модулю ещё не подключён.
...
- **AI:** OpenAI Python SDK и Responses API; в отдельном модуле по умолчанию задана GPT-4.1 mini. Ключ нужен только для проверки AI-объяснений и хранится на серверной стороне.
...
flowchart LR
    ...
    C --> F["Серверный модуль OpenAI"]
    D --> G["Резервное HTTP-объяснение"]
```
...
Проверка реальных OpenAI-ответов **не требуется для основного UI-сценария**. При наличии собственного `OPENAI_API_KEY` в локальном `.env` можно выполнить `.\.venv\Scripts\python.exe scripts/check.py --live-openai`: команда делает пять платных запросов на синтетических факторах и сохраняет отчёт в `outputs/`.
...
Исходные Excel не отправляются в OpenAI API. Внешний сервис нужен только отдельному модулю объяснений по рассчитанным факторам; без ключа расчёт и интерфейс работают. Файлы `.env`, `data/private/`, `data/raw/` и производные результаты `outputs/` исключены из Git.
...
- HTTP-объяснение в UI пока резервное, хотя отдельный модуль OpenAI и его проверки есть. Черновики хранятся только в памяти сервера и исчезают после его перезапуска.
