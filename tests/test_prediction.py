...
def test_monthly_stockout_signal_is_only_a_hypothesis():
    data = monthly_input()
    data["stockout_signals"] = [{"sku": "A", "month": "2026-08", "source": "monthly_stock"}]
    row = recommendation(data)
    assert "suspected_stockout" in row["flags"]
    assert row["factors"]["estimated_lost_demand"] == 0
...
def test_invalid_inputs_fail(mutation):
    ...
    if mutation == "duplicate_sku":
        ...
        data["sales"] = [
            {"date": "2026-08-01", "sku": "A", "quantity": 1, "document_id": "same"},
            {"date": "2026-08-01", "sku": "A", "quantity": 1, "document_id": "same"},
        ]
    else:
